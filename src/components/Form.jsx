    ...
    return (
        <form onSubmit={handleSubmit} className=' w-full lg:w-2/3 pb-10 lg:pb-0 '>
            <div className='text-center'>
                <p className='text-slate-500  font-semibold'><span className='text-secondary_color text-2xl '>{t("CONTACT")}</span> {i18n.language === 'ar' || 'fr' ?"":"US"}</p>
                <p className='text-gray-500 text-sm my-1'>{t("If you have question , feel free to get in touch")}</p>
            ...
            <input className='bg-white py-2  rounded-lg outline-none border-[1px] border-solid border-gray-400 my-3 w-full px-3' type='email'
                ...
                onChange={handleChange}
                required placeholder={t('Email')} />
            <div className='py-5'>
                <textarea id='message'
                    name='message'
                    value={formData.message}
                    onChange={handleChange}
                    required cols={30} className='bg-white py-2  rounded-lg outline-none border-[1px] border-solid border-gray-400 w-full px-3' placeholder={t('Message')} />


            </div>
            <div className='text-center lg:text-end mt-4'>
                <button type='submit' className='cursor-pointer border-[1px] rounded-lg border-solid   bg-secondary_color hover:bg-slate-700  text-white py-2 px-20 '>{t("SEND")}</button>
